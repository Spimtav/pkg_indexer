...
* MAX_QUEUED_CONNECTIONS: how many connection requests the server will queue before denying. NOTE: this must be >= the test script's concurrency value, or the harness will fail with an error that the server rejected the connection.
...
After seeing this relatively fast (for Python) speed, I decided to keep the original implementation and not upgrade it.  Here are some reasons why I felt that this was sufficient:
